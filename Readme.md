...
**[기대 결론 (Expected Conclusion)]**
* **장르 특화 긍정/부정 핵심 요인 정의:** 두 게임의 리뷰 텍스트 분석을 통해 모바일 MOBA 유저들이 평소 느끼는 일반적인 만족 요인(예: 빠른 게임 템포, 소셜 플레이 편의성 등)과 불만족 요인(예: 네트워크 지연, 불공정한 매칭 등)의 핵심 줄기 2가지를 데이터 기반으로 확정한다.
* **특정 시점(날짜)의 이슈 및 문제점 원인 규명:** 월별/주별 부정 비율 그래프에서 비정상적으로 스파이크가 튄 특정한 날을 콕 집어 타겟팅한다. 해당 날짜에 올라온 부정 리뷰 키워드를 집중 분석함으로써 "대규모 패치 이후 발생한 안드로이드 특정 OS 크래시 버그" 또는 "시즌 리셋 당일 MMR 매칭 알고리즘 붕괴" 등 당시 인게임 내에서 실시간으로 일어났던 구체적인 문제점을 명확하게 파악해 낸다.
* **데이터 기반의 라이브 QA 개선 가이드 확보:** 특정 업데이트 시점의 여론 데이터 변동 추이를 확인함으로써, 향후 경쟁 장르 게임의 빌드 업데이트 및 라이브 서비스 운영 시 유저 이탈을 방지하기 위해 최우선으로 검증해야 할 기술 QA 및 시스템 기획적 개선 포인트를 도출한다.
### 2. 데이터 및 전처리 파이프라인
